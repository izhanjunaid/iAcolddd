...
export class EnhanceAccounts1729700200000 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_accounts_financial_statement";`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_accounts_sub_category";`,
    );

    // Remove columns
    await queryRunner.query(
      `ALTER TABLE "accounts" DROP COLUMN IF EXISTS "allow_direct_posting";`,
    );
    await queryRunner.query(
      `ALTER TABLE "accounts" DROP COLUMN IF EXISTS "require_project";`,
    );
    await queryRunner.query(
      `ALTER TABLE "accounts" DROP COLUMN IF EXISTS "is_depreciable";`,
    );
    await queryRunner.query(
      `ALTER TABLE "accounts" DROP COLUMN IF EXISTS "is_bank_account";`,
    );
    await queryRunner.query(
      `ALTER TABLE "accounts" DROP COLUMN IF EXISTS "is_cash_account";`,
    );
    await queryRunner.query(
      `ALTER TABLE "accounts" DROP COLUMN IF EXISTS "display_order";`,
    );
    await queryRunner.query(
      `ALTER TABLE "accounts" DROP COLUMN IF EXISTS "statement_section";`,
    );
    await queryRunner.query(
      `ALTER TABLE "accounts" DROP COLUMN IF EXISTS "financial_statement";`,
    );
    await queryRunner.query(
      `ALTER TABLE "accounts" DROP COLUMN IF EXISTS "sub_category";`,
    );
