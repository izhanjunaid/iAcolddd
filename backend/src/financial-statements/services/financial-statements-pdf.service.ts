...
export class FinancialStatementsPdfService {
  ...
  async generateIncomeStatementPdf(
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.generatePeriodSubtitle(
          doc,
          incomeStatement.periodStart,
          incomeStatement.periodEnd,
        );
   ...
   */
  async generateFinancialAnalysisPdf(
  ...
  private generateHeader(doc: PDFKit.PDFDocument, title: string): void {
    ...
    doc
      ...
      .fontSize(10)
      .font('Helvetica')
      .text('Integrated Accounting & Financial Reporting System', {
        align: 'center',
      })
      .moveDown(1);
  ...
  private generateBalanceSheetSubtitle(
    ...
    doc
      ...
      .font('Helvetica')
      .text(
        `As of ${format(new Date(balanceSheet.periodEnd), 'MMMM dd, yyyy')}`,
        { align: 'center' },
      )
      .moveDown(0.3);

    const statusText = balanceSheet.isBalanced
      ? '✓ BALANCED'
      : '⚠ NOT BALANCED';
  ...
  private generatePeriodSubtitle(
    doc: PDFKit.PDFDocument,
    startDate: Date,
    endDate: Date,
  ): void {
  ...
  private generateFooter(doc: PDFKit.PDFDocument): void {
    ...
    doc
      ...
      .text(`Generated on: ${format(new Date(), 'MMMM dd, yyyy - HH:mm')}`, {
        align: 'center',
      });
  ...
  private generateBalanceSheetContent(
    doc: PDFKit.PDFDocument,
    bs: BalanceSheet,
  ): void {
    const startY = doc.y;
    ...
    if (bs.assets.currentAssets) {
      doc
        .fontSize(11)
        .font('Helvetica-Bold')
        .text('Current Assets', leftCol, assetY);
      assetY += 20;
      assetY = this.renderLineItems(
        doc,
        bs.assets.currentAssets.lineItems,
        leftCol,
        assetY,
        colWidth,
      );
      ...
      doc.text(
        this.formatCurrency(bs.assets.currentAssets.subtotal),
        leftCol + colWidth - 100,
        assetY,
    ...
    if (bs.assets.nonCurrentAssets) {
      doc
        .fontSize(11)
        .font('Helvetica-Bold')
        .text('Non-Current Assets', leftCol, assetY);
      assetY += 20;
      assetY = this.renderLineItems(
        doc,
        bs.assets.nonCurrentAssets.lineItems,
        leftCol,
        assetY,
        colWidth,
      );
      ...
      doc.text(
        this.formatCurrency(bs.assets.nonCurrentAssets.subtotal),
        leftCol + colWidth - 100,
        assetY,
        { width: 100, align: 'right' },
      );
    ...
    doc.text(
      this.formatCurrency(bs.assets.totalAssets),
      leftCol + colWidth - 100,
      assetY,
      { width: 100, align: 'right' },
    );
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#1e40af')
      .text('LIABILITIES & EQUITY', rightCol, liabY);
    doc.fillColor('#000000');
    liabY += 30;
    ...
    if (bs.liabilities.currentLiabilities) {
      doc
        .fontSize(11)
        .font('Helvetica-Bold')
        .text('Current Liabilities', rightCol, liabY);
      liabY += 20;
      liabY = this.renderLineItems(
        doc,
        bs.liabilities.currentLiabilities.lineItems,
        rightCol,
        liabY,
        colWidth,
      );
      ...
      doc.text(
        this.formatCurrency(bs.liabilities.currentLiabilities.subtotal),
        rightCol + colWidth - 100,
        liabY,
        { width: 100, align: 'right' },
      );
      liabY += 25;
    ...
    if (bs.liabilities.nonCurrentLiabilities) {
      doc
        .fontSize(11)
        .font('Helvetica-Bold')
        .text('Non-Current Liabilities', rightCol, liabY);
      liabY += 20;
      liabY = this.renderLineItems(
        doc,
        bs.liabilities.nonCurrentLiabilities.lineItems,
        rightCol,
        liabY,
        colWidth,
      );
      ...
      doc.text(
        this.formatCurrency(bs.liabilities.nonCurrentLiabilities.subtotal),
        rightCol + colWidth - 100,
        liabY,
        { width: 100, align: 'right' },
      );
    ...
    doc.text(
      this.formatCurrency(bs.liabilities.totalLiabilities),
      rightCol + colWidth - 100,
      liabY,
      { width: 100, align: 'right' },
    );
    ...
    if (bs.equity.shareCapital) {
      liabY = this.renderLineItems(
        doc,
        bs.equity.shareCapital.lineItems,
        rightCol,
        liabY,
        colWidth,
      );
    ...
    doc.text(
      this.formatCurrency(bs.equity.retainedEarnings),
      rightCol + colWidth - 100,
      liabY,
      { width: 100, align: 'right' },
    );
    ...
    doc.text(
      this.formatCurrency(bs.equity.currentYearProfit),
      rightCol + colWidth - 100,
      liabY,
      { width: 100, align: 'right' },
    );
    ...
    doc.text(
      this.formatCurrency(bs.equity.totalEquity),
      rightCol + colWidth - 100,
      liabY,
      { width: 100, align: 'right' },
    );
    ...
    doc.text(
      this.formatCurrency(
        bs.liabilities.totalLiabilities + bs.equity.totalEquity,
      ),
      rightCol + colWidth - 100,
      liabY,
      { width: 100, align: 'right' },
  ...
  private generateBalanceSheetMetrics(
    doc: PDFKit.PDFDocument,
    bs: BalanceSheet,
  ): void {
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#1e40af')
      .text('FINANCIAL METRICS', 50, 50);
    doc.fillColor('#000000').moveDown(1);

    const metrics = [
      {
        label: 'Working Capital',
        value: `PKR ${this.formatCurrency(bs.metrics.workingCapital)}`,
      },
      { label: 'Current Ratio', value: bs.metrics.currentRatio.toFixed(2) },
      { label: 'Quick Ratio', value: bs.metrics.quickRatio.toFixed(2) },
      {
        label: 'Debt-to-Equity Ratio',
        value: bs.metrics.debtToEquityRatio.toFixed(2),
      },
      {
        label: 'Return on Assets (ROA)',
        value: `${bs.metrics.returnOnAssets.toFixed(2)}%`,
      },
      {
        label: 'Return on Equity (ROE)',
        value: `${bs.metrics.returnOnEquity.toFixed(2)}%`,
      },
    ];
    ...
    metrics.forEach((metric) => {
      doc
        .font('Helvetica-Bold')
        .text(metric.label, 50, metricY, { width: 200, continued: true });
  ...
  private generateIncomeStatementContent(
    doc: PDFKit.PDFDocument,
    is: IncomeStatement,
  ): void {
    const leftMargin = 50;
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#1e40af')
      .text('REVENUE', leftMargin, currentY);
    doc.fillColor('#000000');
    currentY += 25;
    ...
    currentY = this.renderTotal(
      doc,
      'Total Revenue',
      is.revenue.totalRevenue,
      leftMargin,
      currentY,
      pageWidth,
      '#1e40af',
    ...
    if (is.costOfGoodsSold.items.length > 0) {
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fillColor('#1e40af')
        .text('COST OF GOODS SOLD', leftMargin, currentY);
      doc.fillColor('#000000');
      currentY += 25;

      currentY = this.renderLineItems(
        doc,
        is.costOfGoodsSold.items,
        leftMargin,
        currentY,
        pageWidth,
      );
    ...
    // GROSS PROFIT
    currentY += 20;
    currentY = this.renderHighlight(
      doc,
      'GROSS PROFIT',
      is.grossProfit.amount,
      leftMargin,
      currentY,
      pageWidth,
      '#059669',
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#1e40af')
      .text('OPERATING EXPENSES', leftMargin, currentY);
    doc.fillColor('#000000');
    currentY += 25;
    ...
    if (is.operatingExpenses.administrative) {
      currentY = this.renderSection(
        ...
        is.operatingExpenses.administrative,
        leftMargin,
        currentY,
        pageWidth,
      );
    ...
    currentY = this.renderTotal(
      ...
      is.operatingExpenses.totalOperating,
      leftMargin,
      currentY,
      pageWidth,
    );
    ...
    if (is.otherExpenses.total > 0) {
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fillColor('#1e40af')
        .text('OTHER EXPENSES', leftMargin, currentY);
      doc.fillColor('#000000');
      currentY += 25;
    ...
    doc.text(
      this.formatCurrency(is.tax.taxableIncome),
      leftMargin + pageWidth - 120,
      currentY,
    ...
    doc.text(
      this.formatCurrency(is.tax.taxAmount),
      leftMargin + pageWidth - 120,
      currentY,
      { width: 120, align: 'right' },
    );
  ...
  private generateIncomeStatementMetrics(
    doc: PDFKit.PDFDocument,
    is: IncomeStatement,
  ): void {
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#1e40af')
      .text('PERFORMANCE METRICS', 50);
    doc.fillColor('#000000').moveDown(0.5);

    const metrics = [
      {
        label: 'Gross Profit Margin',
        value: `${is.metrics.grossProfitMargin.toFixed(2)}%`,
      },
      {
        label: 'Operating Margin',
        value: `${is.metrics.operatingMargin.toFixed(2)}%`,
      },
      {
        label: 'Net Profit Margin',
        value: `${is.metrics.netProfitMargin.toFixed(2)}%`,
      },
      {
        label: 'Return on Sales',
        value: `${is.metrics.returnOnSales.toFixed(2)}%`,
      },
      {
        label: 'Expense Ratio',
        value: `${is.metrics.expenseRatio.toFixed(2)}%`,
      },
    ];
  ...
  private generateCashFlowContent(
    doc: PDFKit.PDFDocument,
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#3b82f6')
      .text('CASH FLOWS FROM OPERATING ACTIVITIES', leftMargin, currentY);
    doc.fillColor('#000000');
    currentY += 25;
    ...
    doc
      .fontSize(10)
      .font('Helvetica-Bold')
      .text('Net Income', leftMargin, currentY);
    doc.text(
      this.formatCurrency(cf.operatingActivities.netIncome),
      leftMargin + pageWidth - 120,
      currentY,
      { width: 120, align: 'right' },
    );
    ...
    if (cf.operatingActivities.adjustments.length > 0) {
      doc
        .fontSize(9)
        .font('Helvetica-Bold')
        .text('Adjustments to reconcile net income:', leftMargin, currentY);
      currentY += 18;
      currentY = this.renderLineItems(
        doc,
        cf.operatingActivities.adjustments,
        leftMargin,
        currentY,
        pageWidth,
      );
    ...
    if (cf.operatingActivities.workingCapitalChanges.length > 0) {
      doc
        .fontSize(9)
        .font('Helvetica-Bold')
        .text('Changes in working capital:', leftMargin, currentY);
      currentY += 18;
      currentY = this.renderLineItems(
        doc,
        cf.operatingActivities.workingCapitalChanges,
        leftMargin,
        currentY,
        pageWidth,
      );
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#8b5cf6')
      .text('CASH FLOWS FROM INVESTING ACTIVITIES', leftMargin, currentY);
    doc.fillColor('#000000');
    currentY += 25;
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#059669')
      .text('CASH FLOWS FROM FINANCING ACTIVITIES', leftMargin, currentY);
    doc.fillColor('#000000');
    currentY += 25;
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#1e40af')
      .text('CASH SUMMARY', leftMargin, currentY);
    doc.fillColor('#000000');
    currentY += 25;

    const netChangeColor =
      cf.cashSummary.netCashChange >= 0 ? '#059669' : '#dc2626';
    doc.fontSize(10).font('Helvetica-Bold').fillColor(netChangeColor);
    doc.text('Net Change in Cash', leftMargin, currentY);
    doc.text(
      this.formatCurrency(cf.cashSummary.netCashChange),
      leftMargin + pageWidth - 120,
      currentY,
      { width: 120, align: 'right' },
    );
    ...
    doc.text(
      this.formatCurrency(cf.cashSummary.cashBeginning),
      leftMargin + pageWidth - 120,
      currentY,
      { width: 120, align: 'right' },
    );
    ...
    doc.text(
      this.formatCurrency(cf.cashSummary.cashEnding),
      leftMargin + pageWidth - 120,
      currentY,
      { width: 120, align: 'right' },
    );
  ...
  private generateCashFlowMetrics(
    doc: PDFKit.PDFDocument,
    cf: CashFlowStatement,
  ): void {
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#1e40af')
      .text('CASH FLOW METRICS', 50);
    doc.fillColor('#000000').moveDown(0.5);

    const metrics = [
      {
        label: 'Operating Cash Flow Ratio',
        value: cf.metrics.operatingCashFlowRatio.toFixed(2),
      },
      {
        label: 'Free Cash Flow',
        value: `PKR ${this.formatCurrency(cf.metrics.freeCashFlow)}`,
      },
      {
        label: 'Cash Flow Margin',
        value: `${cf.metrics.cashFlowMargin.toFixed(2)}%`,
      },
    ];
  ...
  private generateFinancialAnalysisContent(
    doc: PDFKit.PDFDocument,
    analysis: FinancialAnalysis,
  ): void {
    const leftMargin = 50;
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#3b82f6')
      .text('LIQUIDITY RATIOS', leftMargin, currentY);
    doc.fillColor('#000000');
    currentY += 25;

    currentY = this.renderRatio(
      doc,
      'Current Ratio',
    ...
    currentY = this.renderCurrencyMetric(
      doc,
      'Working Capital',
      analysis.liquidity.workingCapital,
      leftMargin,
      currentY,
    );
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#059669')
      .text('PROFITABILITY RATIOS', leftMargin, currentY);
    doc.fillColor('#000000');
    currentY += 25;

    currentY = this.renderPercentageMetric(
      doc,
      'Gross Profit Margin',
    ...
    currentY = this.renderPercentageMetric(
      ...
      analysis.profitability.returnOnInvestment,
      leftMargin,
      currentY,
    );
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#8b5cf6')
      .text('EFFICIENCY RATIOS', leftMargin, currentY);
    doc.fillColor('#000000');
    currentY += 25;
    ...
    currentY = this.renderRatio(
      doc,
      'Asset Turnover',
      analysis.efficiency.assetTurnover,
      '1.0x',
      leftMargin,
      currentY,
    );
    if (analysis.efficiency.inventoryTurnover !== undefined) {
      currentY = this.renderRatio(
        doc,
        'Inventory Turnover',
        analysis.efficiency.inventoryTurnover,
        '-',
        leftMargin,
        currentY,
      );
    ...
    if (analysis.efficiency.receivablesTurnover !== undefined) {
      currentY = this.renderRatio(
        ...
        analysis.efficiency.receivablesTurnover,
        '-',
        leftMargin,
        currentY,
      );
    ...
    if (analysis.efficiency.payablesTurnover !== undefined) {
      currentY = this.renderRatio(
        doc,
        'Payables Turnover',
        analysis.efficiency.payablesTurnover,
        '-',
        leftMargin,
        currentY,
      );
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#f59e0b')
      .text('SOLVENCY RATIOS', leftMargin, currentY);
    doc.fillColor('#000000');
    currentY += 25;
    ...
    currentY = this.renderRatio(
      doc,
      'Debt-to-Assets',
      analysis.solvency.debtToAssets,
      '≤ 0.4',
      leftMargin,
      currentY,
    );
    currentY = this.renderRatio(
      doc,
      'Debt-to-Equity',
      analysis.solvency.debtToEquity,
      '≤ 1.0',
      leftMargin,
      currentY,
    );
    ...
    if (analysis.solvency.interestCoverage !== undefined) {
      currentY = this.renderRatio(
        doc,
        'Interest Coverage',
        analysis.solvency.interestCoverage,
        '≥ 3.0x',
        leftMargin,
        currentY,
      );
  ...
  private renderLineItems(
    ...
    items.forEach((item) => {
      ...
      doc.text(this.formatCurrency(item.amount), x + width - 120, y, {
        width: 120,
        align: 'right',
      });
      y += 15;
  ...
  private renderTotal(
    ...
    doc.text(label, x, y);
    doc.text(this.formatCurrency(amount), x + width - 120, y, {
      width: 120,
      align: 'right',
    });
    doc.fillColor('#000000');
  ...
  private renderRatio(
    ...
    doc
      .font('Helvetica')
      .text(value.toFixed(2), x + 250, y, { width: 100, align: 'right' });
    doc
      .fontSize(8)
      .fillColor('#64748b')
      .text(`Target: ${benchmark}`, x + 360, y, { width: 150 });
    doc.fillColor('#000000');
    return y + 22;
  }

  private renderPercentageMetric(
    doc: PDFKit.PDFDocument,
    label: string,
    value: number,
    x: number,
  ...
  private renderCurrencyMetric(
    ...
    doc
      .font('Helvetica')
      .text(`PKR ${this.formatCurrency(value)}`, x + 250, y, {
        width: 200,
        align: 'right',
      });
    return y + 22;
