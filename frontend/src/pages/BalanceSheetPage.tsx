...
import { FileText, Download, CheckCircle, AlertCircle, TrendingUp, FileDown, PieChart as PieChartIcon, BarChart3, ArrowRightLeft } from 'lucide-react';
  ...
  const generateStatement = async () => {
    ...
    try {
      const data = await financialStatementsService.generateBalanceSheet({
        ...
        postedOnly: true,
        includeComparison,
        previousPeriodEnd: includeComparison ? previousPeriodEnd : undefined,
      });
    ...
    return (
      <>
        {section.lineItems.map((item: any, index: number) => (
          ...
          >
            <TableCell
              className={`${item.level === 1 ? 'pl-6' : item.level === 2 ? 'pl-12' : isSubsection ? 'pl-8' : 'pl-2'} ${item.isBold ? 'font-semibold' : ''}`}
            ...
            {balanceSheet?.isComparative && (
              <>
                <TableCell className="text-right tabular-nums text-muted-foreground">
                  {formatCurrency(item.previousAmount || 0)}
                </TableCell>
                <TableCell className={`text-right tabular-nums ${(item.amount - (item.previousAmount || 0)) > 0 ? 'text-green-600' :
                  (item.amount - (item.previousAmount || 0)) < 0 ? 'text-red-600' : 'text-muted-foreground'
                  }`}>
                  {formatCurrency(item.amount - (item.previousAmount || 0))}
                </TableCell>
              </>
            )}
          </TableRow>
              ...
              <div className="flex flex-col gap-2">
                ...
                {includeComparison && (
                  <Input
                    ...
                    onChange={(e) => setPreviousPeriodEnd(e.target.value)}
          ...
          <>
            ...
            <div className="flex space-x-1 rounded-xl bg-slate-100 p-1 mb-6 w-fit">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-colors ${activeTab === 'report' ? 'bg-white shadow text-primary' : 'text-slate-600 hover:text-slate-900'
                  }`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-colors ${activeTab === 'charts' ? 'bg-white shadow text-primary' : 'text-slate-600 hover:text-slate-900'
                  }`}
              >
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow className="border-b-2 border-primary">
                            ...
                            <TableHead className="font-bold text-base text-right tabular-nums">
                              {balanceSheet.isComparative ? `Amount (${new Date(balanceSheet.periodEnd).getFullYear()})` : 'Amount (PKR)'}
                            </TableHead>
                            {balanceSheet.isComparative && (
                              <>
                                <TableHead className="font-bold text-base text-right tabular-nums">
                        ...
                        <TableBody>
                          ...
                          <TableRow className="bg-slate-100 hover:bg-slate-100">
                            <TableCell colSpan={balanceSheet.isComparative ? 4 : 2} className="font-bold text-lg py-4" style={{ fontFamily: 'Georgia, serif' }}>
                              ASSETS
                            </TableCell>
                          </TableRow>
                          ...
                          <TableRow className="bg-slate-50 hover:bg-slate-50">
                            <TableCell colSpan={balanceSheet.isComparative ? 4 : 2} className="font-semibold text-base py-3 pl-4">
                              Non-Current Assets
                            </TableCell>
                          </TableRow>
                            ...
                            {balanceSheet.isComparative && (
                              <>
                                <TableCell className="text-right font-semibold tabular-nums text-muted-foreground">
                                  {formatCurrency(balanceSheet.assets.nonCurrentAssets.previousSubtotal || 0)}
                                </TableCell>
                                <TableCell className={`text-right font-semibold tabular-nums ${(balanceSheet.assets.nonCurrentAssets.subtotal - (balanceSheet.assets.nonCurrentAssets.previousSubtotal || 0)) > 0 ? 'text-green-600' :
                                  (balanceSheet.assets.nonCurrentAssets.subtotal - (balanceSheet.assets.nonCurrentAssets.previousSubtotal || 0)) < 0 ? 'text-red-600' : 'text-muted-foreground'
                                  }`}>
                                  {formatCurrency(balanceSheet.assets.nonCurrentAssets.subtotal - (balanceSheet.assets.nonCurrentAssets.previousSubtotal || 0))}
                                </TableCell>
                              </>
                            )}
                          </TableRow>
                          ...
                          <TableRow className="border-t border-slate-300 bg-slate-50">
                            ...
                            {balanceSheet.isComparative && (
                              <>
                                <TableCell className="text-right font-semibold tabular-nums text-muted-foreground">
                                  {formatCurrency(balanceSheet.assets.currentAssets.previousSubtotal || 0)}
                                </TableCell>
                                <TableCell className={`text-right font-semibold tabular-nums ${(balanceSheet.assets.currentAssets.subtotal - (balanceSheet.assets.currentAssets.previousSubtotal || 0)) > 0 ? 'text-green-600' :
                                  (balanceSheet.assets.currentAssets.subtotal - (balanceSheet.assets.currentAssets.previousSubtotal || 0)) < 0 ? 'text-red-600' : 'text-muted-foreground'
                                  }`}>
                                  {formatCurrency(balanceSheet.assets.currentAssets.subtotal - (balanceSheet.assets.currentAssets.previousSubtotal || 0))}
                                </TableCell>
                              </>
                            )}
                          </TableRow>
                          ...
                          <TableRow className="border-y-2 border-primary bg-blue-50">
                            ...
                            {balanceSheet.isComparative && (
                              <>
                                <TableCell className="text-right font-bold text-base tabular-nums py-4 text-muted-foreground">
                                  {formatCurrency(balanceSheet.assets.previousTotalAssets || 0)}
                                </TableCell>
                                <TableCell className={`text-right font-bold text-base tabular-nums py-4 ${(balanceSheet.assets.totalAssets - (balanceSheet.assets.previousTotalAssets || 0)) > 0 ? 'text-green-700' :
                                  (balanceSheet.assets.totalAssets - (balanceSheet.assets.previousTotalAssets || 0)) < 0 ? 'text-red-700' : 'text-muted-foreground'
                                  }`}>
                                  {formatCurrency(balanceSheet.assets.totalAssets - (balanceSheet.assets.previousTotalAssets || 0))}
                                </TableCell>
                              </>
                            )}
                          </TableRow>
                          ...
                          <TableRow className="bg-slate-100 hover:bg-slate-100">
                            <TableCell colSpan={balanceSheet.isComparative ? 4 : 2} className="font-bold text-lg py-4 pt-6" style={{ fontFamily: 'Georgia, serif' }}>
                              EQUITY AND LIABILITIES
                            </TableCell>
                          </TableRow>
                          ...
                          <TableRow>
                            ...
                            {balanceSheet.isComparative && (
                              <>
                                <TableCell className="text-right tabular-nums text-muted-foreground">
                                  {formatCurrency(balanceSheet.equity.previousRetainedEarnings || 0)}
                                </TableCell>
                                <TableCell className={`text-right tabular-nums ${(balanceSheet.equity.retainedEarnings - (balanceSheet.equity.previousRetainedEarnings || 0)) > 0 ? 'text-green-600' :
                                  (balanceSheet.equity.retainedEarnings - (balanceSheet.equity.previousRetainedEarnings || 0)) < 0 ? 'text-red-600' : 'text-muted-foreground'
                                  }`}>
                                  {formatCurrency(balanceSheet.equity.retainedEarnings - (balanceSheet.equity.previousRetainedEarnings || 0))}
                                </TableCell>
                              </>
                            )}
                          </TableRow>
                          <TableRow>
                            ...
                            {balanceSheet.isComparative && (
                              <>
                                <TableCell className="text-right tabular-nums text-muted-foreground">
                                  {formatCurrency(balanceSheet.equity.previousCurrentYearProfit || 0)}
                                </TableCell>
                                <TableCell className={`text-right tabular-nums ${(balanceSheet.equity.currentYearProfit - (balanceSheet.equity.previousCurrentYearProfit || 0)) > 0 ? 'text-green-700' :
                                  (balanceSheet.equity.currentYearProfit - (balanceSheet.equity.previousCurrentYearProfit || 0)) < 0 ? 'text-red-700' : 'text-muted-foreground'
                                  }`}>
                                  {formatCurrency(balanceSheet.equity.currentYearProfit - (balanceSheet.equity.previousCurrentYearProfit || 0))}
                                </TableCell>
                              </>
                            )}
                          </TableRow>
                          <TableRow className="border-t border-slate-300 bg-slate-50">
                            ...
                            {balanceSheet.isComparative && (
                              <>
                                <TableCell className="text-right font-semibold tabular-nums text-muted-foreground">
                                  {formatCurrency(balanceSheet.equity.previousTotalEquity || 0)}
                                </TableCell>
                                <TableCell className={`text-right font-semibold tabular-nums ${(balanceSheet.equity.totalEquity - (balanceSheet.equity.previousTotalEquity || 0)) > 0 ? 'text-green-600' :
                                  (balanceSheet.equity.totalEquity - (balanceSheet.equity.previousTotalEquity || 0)) < 0 ? 'text-red-600' : 'text-muted-foreground'
                                  }`}>
                                  {formatCurrency(balanceSheet.equity.totalEquity - (balanceSheet.equity.previousTotalEquity || 0))}
                                </TableCell>
                              </>
                            )}
                          </TableRow>
                          ...
                          <TableRow className="bg-slate-50 hover:bg-slate-50">
                            <TableCell colSpan={balanceSheet.isComparative ? 4 : 2} className="font-semibold text-base py-3 pl-4 pt-6">
                              Non-Current Liabilities
                            </TableCell>
                          </TableRow>
                          {balanceSheet.liabilities.nonCurrentLiabilities.lineItems.length > 0 ? (
                            <>
                              ...
                              <TableRow className="border-t border-slate-300 bg-slate-50">
                                ...
                                {balanceSheet.isComparative && (
                                  <>
                                    <TableCell className="text-right font-semibold tabular-nums text-muted-foreground">
                                      {formatCurrency(balanceSheet.liabilities.nonCurrentLiabilities.previousSubtotal || 0)}
                                    </TableCell>
                                    <TableCell className={`text-right font-semibold tabular-nums ${(balanceSheet.liabilities.nonCurrentLiabilities.subtotal - (balanceSheet.liabilities.nonCurrentLiabilities.previousSubtotal || 0)) > 0 ? 'text-red-600' : // Increase in liability is bad? Context matters. Usually simple variance color.
                                        (balanceSheet.liabilities.nonCurrentLiabilities.subtotal - (balanceSheet.liabilities.nonCurrentLiabilities.previousSubtotal || 0)) < 0 ? 'text-green-600' : 'text-muted-foreground'
                                      }`}>
                                      {formatCurrency(balanceSheet.liabilities.nonCurrentLiabilities.subtotal - (balanceSheet.liabilities.nonCurrentLiabilities.previousSubtotal || 0))}
                                    </TableCell>
                                  </>
                                )}
                              </TableRow>
                            ...
                            <TableRow>
                              <TableCell colSpan={balanceSheet.isComparative ? 4 : 2} className="pl-6 text-muted-foreground italic">
                          ...
                          <TableRow className="border-t border-slate-300 bg-slate-50">
                            ...
                            {balanceSheet.isComparative && (
                              <>
                                <TableCell className="text-right font-semibold tabular-nums text-muted-foreground">
                                  {formatCurrency(balanceSheet.liabilities.currentLiabilities.previousSubtotal || 0)}
                                </TableCell>
                                <TableCell className={`text-right font-semibold tabular-nums ${(balanceSheet.liabilities.currentLiabilities.subtotal - (balanceSheet.liabilities.currentLiabilities.previousSubtotal || 0)) > 0 ? 'text-red-600' :
                                    (balanceSheet.liabilities.currentLiabilities.subtotal - (balanceSheet.liabilities.currentLiabilities.previousSubtotal || 0)) < 0 ? 'text-green-600' : 'text-muted-foreground'
                                  }`}>
                                  {formatCurrency(balanceSheet.liabilities.currentLiabilities.subtotal - (balanceSheet.liabilities.currentLiabilities.previousSubtotal || 0))}
                                </TableCell>
                              </>
                            )}
                          </TableRow>
                            ...
                            {balanceSheet.isComparative && (
                              <>
                                <TableCell className="text-right font-bold tabular-nums text-muted-foreground">
                                  {formatCurrency(balanceSheet.liabilities.previousTotalLiabilities || 0)}
                                </TableCell>
                                <TableCell className={`text-right font-bold tabular-nums ${(balanceSheet.liabilities.totalLiabilities - (balanceSheet.liabilities.previousTotalLiabilities || 0)) > 0 ? 'text-red-700' :
                                    (balanceSheet.liabilities.totalLiabilities - (balanceSheet.liabilities.previousTotalLiabilities || 0)) < 0 ? 'text-green-700' : 'text-muted-foreground'
                                  }`}>
                                  {formatCurrency(balanceSheet.liabilities.totalLiabilities - (balanceSheet.liabilities.previousTotalLiabilities || 0))}
                                </TableCell>
                              </>
                            )}
                          </TableRow>
                          ...
                          <TableRow className="border-y-2 border-primary bg-blue-50">
                            ...
                            {balanceSheet.isComparative && (
                              <>
                                <TableCell className="text-right font-bold text-base tabular-nums py-4 text-muted-foreground">
                                  {formatCurrency((balanceSheet.liabilities.previousTotalLiabilities || 0) + (balanceSheet.equity.previousTotalEquity || 0))}
                                </TableCell>
                                <TableCell className={`text-right font-bold text-base tabular-nums py-4 ${((balanceSheet.liabilities.totalLiabilities + balanceSheet.equity.totalEquity) - ((balanceSheet.liabilities.previousTotalLiabilities || 0) + (balanceSheet.equity.previousTotalEquity || 0))) > 0 ? 'text-indigo-700' : 'text-indigo-700'
                                  }`}>
                                  {formatCurrency((balanceSheet.liabilities.totalLiabilities + balanceSheet.equity.totalEquity) - ((balanceSheet.liabilities.previousTotalLiabilities || 0) + (balanceSheet.equity.previousTotalEquity || 0)))}
                                </TableCell>
                              </>
                            )}
                          </TableRow>
        ...
        {activeTab === 'charts' && balanceSheet && (
          <div className="space-y-6 animate-in fade-in duration-500">
                ...
                <CardContent className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={[
                          { name: 'Current Assets', value: balanceSheet.assets.currentAssets.subtotal },
                          { name: 'Non-Current Assets', value: balanceSheet.assets.nonCurrentAssets.subtotal },
                        ]}
                        cx="50%"
                        cy="50%"
                        innerRadius={60}
                        ...
                        label
                      >
                        <Cell key="cell-0" fill="#0ea5e9" /> {/* sky-500 */}
              ...
              <Card>
                ...
                <CardContent className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={[
                        { name: 'Liabilities', value: balanceSheet.liabilities.totalLiabilities },
                        { name: 'Equity', value: balanceSheet.equity.totalEquity },
                      ]}
                      layout="vertical"
                      margin={{ left: 20 }}
                    >
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis type="number" tickFormatter={(value) => `Rs ${(value / 1000).toFixed(0)}k`} />
                      <YAxis dataKey="name" type="category" width={80} />
                      <RechartsTooltip formatter={(value: number) => formatCurrency(value)} />
                      <Legend />
                      <Bar dataKey="value" fill="#f43f5e" name="Amount" radius={[0, 4, 4, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
            ...
            <div className="grid grid-cols-1 gap-6">
              <Card>
                <CardHeader>
                  <CardTitle>Current Assets Breakdown</CardTitle>
                </CardHeader>
                <CardContent className="h-96">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={balanceSheet.assets.currentAssets.lineItems.filter(i => i.level === 1)}
                      margin={{ bottom: 40 }}
                    >
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="label" angle={-45} textAnchor="end" height={80} interval={0} fontSize={12} />
                      <YAxis />
                      <RechartsTooltip formatter={(value: number) => formatCurrency(value)} />
